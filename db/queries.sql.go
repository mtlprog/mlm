...
const getPendingReport = `-- name: GetPendingReport :one
...
func (q *Queries) GetPendingReport(ctx context.Context) (Report, error) {
	row := q.db.QueryRow(ctx, getPendingReport)
	var i Report
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
